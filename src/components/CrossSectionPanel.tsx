...
export function CrossSectionPanel({ results, solarAz, crossAz, timeUTC, onEnlarge, isMain }: { results: ThresholdResult[]; solarAz: number; crossAz: number; timeUTC: string; onEnlarge?: () => void; isMain?: boolean }) {
  const w = 900;
  const h = isMain ? 560 : 300;
  const m = { l: 64, r: 18, t: 24, b: 56 };
  ...
  return <div className='panel'><div className='panelHeader'><h3>Vertical Cross-Section</h3>{onEnlarge && <button onClick={onEnlarge}>Enlarge</button>}</div><svg preserveAspectRatio='xMidYMid meet' viewBox={`0 0 ${w} ${h}`}>
    ...
    {xt.map(t => <g key={`xt-${t}`}><line x1={sx(t)} x2={sx(t)} y1={h - m.b} y2={h - m.b + 6} stroke='#334155' /><text x={sx(t)} y={h - 30} fontSize='11' textAnchor='middle'>{t}</text></g>)}
    ...
    <text x={w / 2} y={h - 8} textAnchor='middle' fontSize='12'>Distance along terminator-normal [km]</text><text transform={`translate(16 ${h / 2}) rotate(-90)`} fontSize='12' textAnchor='middle'>Altitude [km]</text>
