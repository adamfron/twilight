...
import { PanelKey, Station, ThresholdDef, ThresholdKey, TimeMode } from './types';
...
export default function App() {
  const [stationId, setStationId] = useState(stations[0].id); const [customLat,setCustomLat]=useState(52.0); const [customLon,setCustomLon]=useState(5.0); const [customAltM,setCustomAltM]=useState(0); const [date, setDate] = useState('2026-05-19'); const [manualTime, setManualTime] = useState('04:00'); const [mode, setMode] = useState<TimeMode>('manual'); const [anchor, setAnchor] = useState('0');
  ...
  const [showIonosphereBands, setShowIonosphereBands] = useState(true);
  const [showTwilightShading, setShowTwilightShading] = useState(true);
  const presetStation = stations.find(s => s.id === stationId) ?? stations[0]; const station: Station = stationId === 'custom' ? { id:'custom', name:'Custom position', latitude:customLat, longitude:customLon, altitudeM:customAltM, timeZone:'UTC' } : presetStation; const anchorDef = defs.find(d => d.angleDeg === Number(anchor))!;
  ...
  const panels = { cross: <CrossSectionPanel results={filteredResults} solarAz={solar.azimuthDeg} crossAz={crossAz} timeUTC={new Date(activeUtc + 'Z').toISOString()} timeLocal={stationLocalTime} showIonosphereBands={showIonosphereBands} showTwilightShading={showTwilightShading} onEnlarge={main === 'cross' ? undefined : () => setMain('cross')} isMain={main === 'cross'} />, geometry: <SunEarthGeometryPanel crossAz={crossAz} solarAz={solar.azimuthDeg} declinationDeg={solar.declinationDeg} onEnlarge={main === 'geometry' ? undefined : () => setMain('geometry')} isMain={main === 'geometry'} />, map: <LocalMapPanel lat={station.latitude} lon={station.longitude} crossAz={crossAz} frontAz={frontAz} onEnlarge={main === 'map' ? undefined : () => setMain('map')} mapState={mapState} isMain={main === 'map'} />, timeline: <TimelineDiagnosticsPanel timeline={timeline} selectedHour={activeDate.getUTCHours() + activeDate.getUTCMinutes() / 60} elev={solar.elevationDeg} solarAz={solar.azimuthDeg} crossAz={crossAz} frontAz={frontAz} anchor={anchorDef} mode={mode} eventTime={mode === 'manual' ? '' : new Date(activeUtc + 'Z').toISOString().slice(11, 16)} status={status} diagnostics={diagnostics} onEnlarge={main === 'timeline' ? undefined : () => setMain('timeline')} isMain={main === 'timeline'} /> };
  ...
  return <div className='app'><header className='compactHeader'><h1><strong>TWILIGHT</strong> — Terminator Web Interface for Local Inclination Geometry, Heights & Twilight</h1></header>
    <InputBar stations={stations} stationId={stationId} onStation={setStationId} customLat={customLat} customLon={customLon} customAltM={customAltM} onCustomLat={setCustomLat} onCustomLon={setCustomLon} onCustomAltM={setCustomAltM} date={date} onDate={setDate} manualTime={manualTime} onManualTime={setManualTime} mode={mode} onMode={setMode} anchor={anchor} onAnchor={setAnchor} onCompute={onCompute} onHowTo={() => setHowTo(true)} visibleThresholds={visibleThresholds} onToggleThreshold={k => setVisibleThresholds(v => v.includes(k) ? v.filter(x => x !== k) : [...v, k])} />
    ...
    <ControlStrip onExport={onExport} onStep={onStep} onPng={onPng} onCopy={onCopy} status={status} showIonosphereBands={showIonosphereBands} showTwilightShading={showTwilightShading} onToggleIonosphere={() => setShowIonosphereBands(v => !v)} onToggleTwilight={() => setShowTwilightShading(v => !v)} /><ReferencesFooter /><HowToUseModal open={howTo} onClose={() => setHowTo(false)} /></div>;
}
